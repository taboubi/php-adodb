...
static void adodb_init_zval(zval *v, char *s)
{
	...
	Z_SET_ISREF_P(v);	// v->is_ref = 0;       // Invalid for PHP5.3
	Z_SET_REFCOUNT_P(v, 1);	// v->refcount = 1;     // Invalid for PHP5.3
}
...
zval		**fields;
	...
	fci.object_ptr = NULL; 	// fci.object_pp = NULL; // Invalid in PHP5.3
